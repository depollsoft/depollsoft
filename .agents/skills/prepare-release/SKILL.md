...
Pitch Perfect Android also releases Wear OS on its dedicated `wear:production` track. Confirm that track is configured before deploying. The watch uses the Android version and the phone build number plus one, so the next phone build must exceed both. Review watch-specific changes and release notes, and validate both phone and watch production builds. Do not retain the obsolete watch bundle on the phone track.
