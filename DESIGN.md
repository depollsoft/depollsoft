...
- Android normal mode keeps rows clean. **Edit Songs** is a title-bar action (pencil) that flips to a Done checkmark, reveals each row's pencil and drag handle, and surfaces Sort beside it; the Settings gear stays on the bar. Dragging uses RecyclerView/ItemTouchHelper and persists once on drop.
