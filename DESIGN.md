...
The pitch-pipe screen is a single custom-drawn face (`PitchInstrumentFace` in PitchPerfectLib, painted by `PitchInstrumentRenderer.kt` / `DPPitchInstrumentView.swift`) with identical geometry on both platforms: face center at 44% of height; ring radius = min(w, h × 0.82) × 0.365; 13 cells on a true circle with the inclusive octave endpoints flanking −90° (step = 360° / 13); cell radius = ring radius × 0.225; the range selector seated in the ring's hole (width = ring × 0.72, row height = ring × 0.145, top at center + ring × 0.20). The nameplate caption sits at the panel's bottom edge.
...
- 56–64dp fully transparent rows over the full-bleed score (lists themselves carry the score pattern on iOS and sit transparent on Android), with 1px hairline dividers. An empty song list uses the same engraved "NO SONGS ON FILE" prompt on both platforms and points directly to Add. Primary text condensed 20–24sp ink; trailing datum mono 14–18sp secondary. Android song titles keep a 20dp leading inset and song keys use the 18sp ceiling. Pressed state = the lit treatment (background flips to `plate-lit`, text to `plate-on-lit`) — a row lights the way a cell does.

### Set List Selector (machined)

- The Songs tab opens with the set list selector above the rows: one round-rect frame (5px radius, 1.5px hairline) holding one position per set list plus a trailing "+", split by 1px interior hairlines, exactly the range selector's construction. 48dp tall, 16dp side margins, 12dp above and 8dp below. Positions share the whole frame while they fit; once they overflow they scroll inside the frame, which stays put, with 24dp end fades as the scroll cue, and the current one is scrolled into view.
- Positions are engraved labels: the list's display name uppercased in Oswald Medium 13sp with 0.16 tracking, 20dp leading and 14dp trailing padding, truncated at 180dp. The current list carries a 10%-ink wash, full-ink text and the range selector's 6dp `plate-lit` dot 8dp inside its leading edge; the others sit in full secondary ink (75% alpha, the range selector's value, falls under AA at 13sp). Tapping a position switches the rows immediately with the selection tick; long-pressing one opens a menu titled with that list's name (rename, duplicate, delete, manage) as a popup on Android and the button's native menu on iOS; "+" (20sp) opens the New set list prompt. `SetListSelector.kt` on Android, `SetListSelectorView.swift` on iOS.
...
- Android normal mode keeps rows clean. **Edit Songs** is a title-bar action (pencil) that flips to a Done checkmark, reveals each row's pencil and drag handle, and surfaces Sort beside it; the Settings gear stays on the bar. Dragging uses DepollSoftCompose's reorder engine (`Reorder.kt`) and persists once on drop.
- Edit mode is about the list's contents: Android's overflow carries **Add songs from another set list…** (retitled "Nothing to add from other set lists" when disabled) and **Manage set lists…**; iOS shows Done alone on the left and Add plus a More (`ellipsis.circle`) menu on the right with Sort Alphabetically, Add songs from another set list… (subtitled "Nothing to add" when disabled) and Manage set lists…, each with an SF Symbol. The list itself is managed from the selector long-press and the Set Lists screen. Rename and New use a name prompt with inline validation; Android's Material dialogs take `ThemeOverlay.Plate.Dialog`, which puts buttons, errors and field strokes in ink, and disables the elevation overlay so the dialog is `plate-surface`, etched not cast. Delete confirms with the song count and, on Android, offers Undo in the Snackbar; Duplicate needs no prompt, names the copy "<name> copy", switches to it and stays in edit mode.
- **Add songs** is a sectioned checklist (one engraved section header per other list, rows in the song-row typography exposed as checkboxes, songs the current list already has left out) confirmed by "Add N songs"; the confirm button and checkmark drop to hairline/secondary ink while disabled (`plate_button_fill` / `plate_button_text`), so nothing is lit that cannot act. **Set Lists** is the manage screen: name plus mono count per row with the lit dot on the current list, drag handles on custom rows only, tap to switch and return, a per-row menu titled with the list's name for Rename / Duplicate / Delete, "+" for a new list.
...
- Android uses a bottom navigation bar (`PlateBottomNavigation`) on compact widths and a navigation rail (`PlateNavigationRail`) at 600dp+; selected items are `plate-ink`, unselected `plate-ink-secondary`; app-bar icons tint through `colorControlNormal`. All labels remain visible and compact items never shift, so label geometry cannot animate after the page. The four lightweight pages remain resident, pager↔navigation updates are equality-guarded, and one standard page transition coordinates the switch. One opaque ground/grain/score bitmap is shared, cached by size, and held in GPU memory behind transparent page content, avoiding stacked full-screen layers. Toolbar actions change only after a page settles. Home-screen widget renders run off the main thread and coalesce, firing only when a note starts or stops sounding. System bars match `plate-surface`; the app bar carries “Pitch Perfect” and an always-visible Settings gear.
...
- The instrument exposes virtualized elements: Android Compose semantics nodes (13 cells + 2 range rows as Buttons with bounds), iOS `UIAccessibilityElement` containers. Spoken names carry full note names ("C sharp, D flat, octave 4") even where the engraving shows only "♯/♭"; range rows report selection; a screen-reader tap sounds the note for 1.5s.
