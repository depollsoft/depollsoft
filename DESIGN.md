...
- The app-owned prompt exposes three full-width choices. Email and Google launch one-provider FirebaseUI 9.1.1 flows directly—no redundant second picker—with a dedicated `AuthTheme` so fields, activated controls, progress, and actions remain visible in light and dark. Facebook bypasses FirebaseUI social completion entirely: Facebook SDK 18.3 handles OAuth, then its access token is exchanged directly with Firebase Auth while the prompt owns busy, cancel, failure, and retry states. The account label tolerates a missing Facebook email.
- FirebaseUI Credential Manager saving remains disabled for Email/Google. Facebook never enters FirebaseUI’s `CredentialSaveActivity`, eliminating its nullable-email/non-null credential-save crash path while preserving Firebase Auth, Firestore attachment, and account sync.
