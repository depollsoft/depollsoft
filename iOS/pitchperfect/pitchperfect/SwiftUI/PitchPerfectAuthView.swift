...
struct PitchPerfectAuthView: View {
    @State private var authService: AuthService
    let onSignIn: () -> Void
    let onDismiss: () -> Void
    @State private var didFinish = false

    init(onSignIn: @escaping () -> Void, onDismiss: @escaping () -> Void) {
        ...
        let service = AuthService(configuration: configuration)
            ...
            .withGoogleSignIn()
            .withFacebookSignIn()
        service.registerProvider(providerWithButton: PitchPerfectAppleProviderUI())
        #if canImport(FirebasePhoneAuthSwiftUI)
