#!/usr/bin/env python3
"""Register a source file with the tagmaster.xcodeproj (legacy explicit-reference project).

Usage: add_source.py <FileName.swift|.m> [--target app|tests]

The file is placed next to an existing sibling in the same group (TMTrackPlayerView.swift for the
app target, TMBehaviorTestSupport.swift for the test target) and added to that target's Sources
...
PROJECT = Path(__file__).resolve().parents[1] / "tagmaster.xcodeproj" / "project.pbxproj"
ANCHORS = {"app": "TMTrackPlayerView.swift", "tests": "TMBehaviorTestSupport.swift"}


def next_ids(text, count):
    used = set(re.findall(r"B4A1B0000000000000000([0-9A-F]{3})", text))
    ids, n = [], 0
    while len(ids) < count:
        candidate = "%03X" % n
        n += 1
        if candidate not in used:
            ids.append("B4A1B0000000000000000" + candidate)
    return ids
