...
struct TMCatalogTagLoader: TMTagLoading {
    func load(_ tagId: Int32, refresh: Bool, completion: @escaping @MainActor (DPTag?) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            // A raise leaves the result nil: a useful, non-diagnostic recovery state.
            let result = TMObjC.catching { DPTag.load(byId: tagId, refresh: refresh) } as? DPTag
