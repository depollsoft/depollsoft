...
namespace BarbershopTags {
  public partial class AudioPlayer : UserControl {
    ...
    private void RefreshSource() {
      ...
      WebRequest wr = _LatestRequest = WebRequest.Create(Source.Uri);
      IsLoading = true;
      ...
      wr.BeginGetResponse(res => {
        UiUtilities.ExecuteOnUiThread(() => {
          try {
            ...
            var file = iso.CreateFile(_LastTempFile = "AudioPlayerTemp" + _InstanceNum + "_" + ++_LastTemp + filename + "." + Source.Type);
            if (Source.Type.EndsWith("mid"))
    ...
    public RemoteLocation Source {
      get { return (RemoteLocation)GetValue(SourceProperty); }
      set { SetValue(SourceProperty, value); }
    }

    // Using a DependencyProperty as the backing store for Source.  This enables animation, styling, binding, etc...
    public static readonly DependencyProperty SourceProperty =
        DependencyProperty.Register("Source", typeof(RemoteLocation), typeof(AudioPlayer), new PropertyMetadata(null, OnSourceChanged));
