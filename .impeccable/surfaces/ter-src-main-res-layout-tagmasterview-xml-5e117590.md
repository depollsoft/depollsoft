...
- Empty pane (`tag_pane_empty.xml` / `TMTagPlaceholderController`): barber-pole watermark, quartet staff at rest (216x96dp; notes in the accent blue #007AA3 light / #5AC8FA dark on both platforms, staff lines at 29% grey), "Pick a tag" in `textAppearanceTitleLarge` / semibold Title2, body in `textAppearanceBodyMedium` on `colorOnSurfaceVariant` / Body on `secondaryLabelColor`, 12dp then 8dp stacking, 480dp max text width, 32dp padding.
