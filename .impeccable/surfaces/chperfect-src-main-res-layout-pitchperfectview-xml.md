...
Direction: The Laboratory Instrument (seed 1d4ba7fb, bolder-register challenger fused with product truth; user-pinned grayscale). The pipe is a custom-drawn radial instrument face; every other screen is panel-mounted hardware in the same world. Clean staff etching is the heritage base; two faint crops from the original artwork (treble/key signature upper-left, bass/key signature lower-right) reconnect it to the icon without restoring the dense wallpaper.
...
Constraints: grayscale only (one luminous value); ads stay in a fixed full-width bottom slot; ♯/♭ glyph cells (physical-pipe convention, user-pinned); NoteHedz/MusiQwik glyph faces preserved; Android song pencils/drag handles appear only in explicit Edit mode; iOS uses opaque grayscale chrome instead of Liquid Glass and its song-editor keyboard must always be dismissible; widget and Wear untouched.
