...
Constraints: grayscale only (one luminous value); ads stay in a fixed full-width bottom slot; ♯/♭ glyph cells (physical-pipe convention, user-pinned); NoteHedz/MusiQwik glyph faces preserved; Android song pencils/drag handles appear only in explicit Edit mode; iOS keeps native Liquid Glass over the full-bleed score, with stable template icons; its song-editor keyboard must always be dismissible; the Android home-screen widget mirrors the circular instrument; Wear remains untouched.
