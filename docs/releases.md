...
Android CI and preview builds reuse installed SDK packages after checking their tools, platform files, and accepted license file. Missing packages use the pinned setup action with a five-minute deadline, followed by the same file checks. They do not run a second license scan and reinstall after setup.
