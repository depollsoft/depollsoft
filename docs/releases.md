...
iOS simulator CI and asset capture use the free standard `macos-26` Apple Silicon runner. Keep compilation and simulator execution sequential to limit memory use. Intel runner probes stalled under extreme load during first boot. CoreSimulator can log `Data Migration Failed` while returning success and allowing native apps to run, so actual XCTest/capture results determine readiness. Nonzero command exits and startup deadlines still fail validation.
