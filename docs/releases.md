...
iOS captures use hosted macOS 26 runners. Pitch Perfect Android captures use hosted Ubuntu; Tag Master Android uses the existing self-hosted macOS pool because hosted Ubuntu runners timed out connecting to its live catalog and the Linux pool has no usable KVM device. A catalog preflight checks connectivity before building. Captures use temporary emulators, with port 5560 reserved for Tag Master, and serialize each app/platform across runs. SDK disk cleanup and KVM configuration run only on disposable hosted runners. Fork PRs run only the release plan tests on a hosted runner. No deployment credentials are passed to capture jobs. iOS deployment uses the existing self-hosted macOS pool. Consider making the release checks required in branch protection.
