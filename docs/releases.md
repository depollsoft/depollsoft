...
Build numbers default to Unix seconds and can be overridden with `--build`. Confirm they exceed the latest uploaded build, including builds that have not shipped. The helper enforces increasing versions/builds against the previous plan and published tags. Store APIs remain the authority for uploads made outside this system. Production build overrides apply only to the selected Gradle module or Xcode scheme, including Pitch Perfect's iOS widget. Existing private preview numbering is unchanged.

Pitch Perfect Android includes its Wear OS companion. Both use the plan's Android version; the phone uses the plan's build number and the watch uses that number plus one. The next release's phone build must exceed both. Configure the dedicated Wear OS track in Play Console before release: Test and release → Advanced settings → Form factors → Wear OS → Manage → Use a dedicated release track for Wear OS. The lane checks that `wear:production` exists before building. It submits the current watch bundle there first, then submits the phone bundle to `production` without retaining the obsolete 2022 watch bundle. Both track submissions must succeed before the Android release record is published. A partial retry reuses each accepted track release independently.
...
Hosted iOS simulator CI and asset capture pin `IOS_SIMULATOR_VERSION=26.5` on the free standard Apple Silicon runner. A missing pinned runtime fails explicitly; local captures use the newest compatible runtime unless this variable is set. Pitch Perfect and Tag Master build concurrently on separate free runners. Each build uses an unbooted disposable simulator destination and enables coverage so Xcode includes coverage metadata. A pre-upload check rejects missing metadata. Each app packages its compiled test products once, preserving executable permissions and framework symlinks in a tar archive. Independent runners download only their app’s products and run `.xctestrun` files directly, without another build or package resolution. Three unit suites and three Tag Master UI groups run in parallel, with a separate owned iPhone SE simulator and result artifact per job. The smaller test device reduces rendering memory on the standard runner; release captures retain their required phone/tablet sizes. Scheduled/manual runs add Pitch Perfect’s UI suite. New Tag Master UI classes automatically run in the remaining-class group; the aggregate check rejects missing suites, duplicate UI cases, failures, and tests over their suite budget. Release captures wait for this complete workflow.

iOS simulator CI and asset capture use the free standard `macos-26` Apple Silicon runner. Build jobs compile before handing binaries to separate test runners; each test runner boots only its own simulator. Intel runner probes stalled under extreme load during first boot. CoreSimulator can log `Data Migration Failed` while returning success and allowing native apps to run, so actual XCTest/capture results determine readiness. Nonzero command exits and startup deadlines still fail validation.

Each iOS test or capture job owns a separate simulator. Release captures can run in parallel on separate GitHub-hosted machines. Release PR captures wait for that commit's iOS CI when its path filters require tests; copy-only release PRs proceed directly. A failed iOS check prevents capture; rerun release assets after fixing or rerunning CI. Xcode builds use two compiler jobs to limit peak memory. A short allocation lock protects device creation and ownership records; builds, boot, tests, and captures do not hold it. Records identify the Actions job's `Runner.Worker` process and its start time, so ownership survives shell-step boundaries and PID reuse cannot preserve abandoned devices. Cleanup removes disposable devices from exited jobs and skips live owners. Cancellation stops only the command's process group and simulator. Shutdown and deletion each have a 30-second limit; a stuck shutdown still attempts deletion. Test simulators explicitly finish boot before XCTest starts. For local cleanup of a known legacy shared CI device, pass its exact UDID as `IOS_LEGACY_SIMULATOR_UDID` when running `scripts/ci/ios_simulator.py`. Hosted jobs do not use this setting.

Tag Master UI suites launch and terminate the app once in class setup before timing individual regressions, to prepare first-launch services on a fresh simulator. Suite startup remains bounded. Each iOS test job interrupts XCTest after its first failed case; other parallel jobs finish and retain their results. It enforces a 30-second duration budget per unit test and 90 seconds per UI case, including app launch, rotation, and accessibility work on free runners. It saves unfiltered logs and reports the slowest cases. After simulator boot, a prebuilt test command has five minutes to start its first case; startup progress is reported every 30 seconds. A failed or hung command gets 15 seconds to stop before its owned process group is killed. Keep each UI test focused on one screen or interaction, check whether controls are already ready before waiting, and wait for the state the assertion needs, such as tappability after rotation. Failed or timed-out runs save process stacks and resource snapshots in the test-results artifact. Profiling starts only after failure so diagnostic sampling cannot compete with a running test. Both UI-test schemes use automatic screenshots instead of screen recording, avoiding XCTest video-finalization stalls during teardown. Failure screenshots and explicit store screenshot attachments remain available. Store captures dismiss SpringBoard notification banners before taking a screenshot and recheck afterward, so first-boot system announcements cannot cover the listing images. The opt-in `StoreScreenshotNotificationTests` case exercises this with a real local notification; run it with `TEST_RUNNER_STORE_NOTIFICATION_TEST=1` and the `pitchperfectUITests` scheme. Store screenshot tours run separately as asset-generation jobs. Unit tests intercept share-controller creation as well as presentation, so Apple's share-extension and link-metadata work cannot spill into later tests. The native share UI test still opens the real system sheet.
...
The iOS API key needs access to both production apps. The existing store records must have their review contact, content rights, age rating, privacy, and advertising answers completed. Store agreements or new required fields can block submission and need correction in the console.

The app owner confirmed that neither app uses non-exempt encryption. Fastlane supplies this declaration for both production App Store submissions and private TestFlight uploads, including retries of an existing build. Production uses `submission_information: {export_compliance_uses_encryption: false}`; TestFlight uses `uses_non_exempt_encryption: false`. Reassess these values if either app's encryption changes. Other store-account answers still require the owner's input.

Release jobs use GitHub-hosted runners. Android captures use `ubuntu-latest` with KVM; iOS captures use `macos-26`. A catalog preflight checks connectivity before building. Captures use temporary emulators, with port 5560 reserved for Tag Master and 5554 for Pitch Perfect, and serialize each app/platform across runs. Each Android capture owns a separate ADB server on an available private port, with emulator auto-discovery disabled. A host lock also protects concurrent local captures. A failed iOS tour resets only its disposable simulator before its one retry. Fork PRs do not execute mobile or release builds. No deployment credentials are passed to capture jobs. Plan validation, PR artifact links, and Android deployment use Linux; iOS deployment uses macOS. Consider making the release checks required in branch protection.
...
Android CI, iOS CI and PR Preview first run `scripts/ci/changed_apps.py` to decide which apps a change touches: a file under one app's own module (`Android/PitchPerfect`, `Android/PitchPerfectWear`, `Android/TagMaster`, `iOS/pitchperfect`, `iOS/tagmaster`) selects that app, while any other file under `Android/` or `iOS/` (shared libraries, Bindroid, depolllib, `iOS/shared`, `pitchperfectlib`, Gradle and workspace files, Fastlane) and the platform's workflow files select both. Manual and scheduled runs select everything. Android CI runs only the selected modules' unit tests and release compiles (every module's tests when shared code changed); iOS CI builds and tests only the selected apps; PR Preview builds, shares and distributes only the selected apps and skips an untouched platform. Android CI compiles the selected production variants in addition to running debug unit tests. This catches missing release resources without requiring signing credentials. Pitch Perfect's production banner ID lives in `src/main/res/values/ad_unit.xml`; debug and private resources override it with Google's test ID.
