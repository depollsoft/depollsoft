...
Hosted iOS simulator CI and asset capture pin `IOS_SIMULATOR_VERSION=26.5` on the 14 GB Apple Silicon runner, verified by a cold-boot/native-app-launch probe. A missing pinned runtime fails explicitly; local captures use the newest compatible runtime unless this variable is set. Prebuilt CI suites run their `.xctestrun` files directly so test startup does not resolve packages again.

iOS simulator CI and asset capture use the paid `macos-26-xlarge` Apple Silicon runner with 14 GB of RAM. The 7 GB ARM runner experienced severe memory pressure and the 14 GB Intel runners stalled under extreme load during first boot. The larger native runner completed boot in about 30 seconds and launched a UIKit app successfully. CoreSimulator can log `Data Migration Failed` while returning success and allowing native apps to run, so actual XCTest/capture results determine readiness. Nonzero command exits and startup deadlines still fail validation.
