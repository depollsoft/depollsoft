...
public class PitchPipeModelTest {
    ...
    public void constructor_initializesCToCNotes_whenIsFromFToFIsFalse() {
        ...
        assertEquals("C-to-C scale should have 12 notes", 12, notes.size());
        ...
        // Verify last note is B4
        Note lastNote = notes.get(11);
        assertEquals("Last note should be B", "B", lastNote.getFriendlyName());
        assertEquals("Last note should be Natural", Accidental.Natural, lastNote.getAccidental());
        assertEquals("Last note should be octave 4", 4, lastNote.getOctave());
    }
    ...
    public void getNotes_returnsCToCScale_byDefault() {
        ...
        // Verify chromatic C-to-C scale: C, C#, D, D#, E, F, F#, G, G#, A, A#, B
        String[] expectedNames = {"C", "C", "D", "D", "E", "F", "F", "G", "G", "A", "A", "B"};
    ...
    public void setIsFromFToF_true_switchesToFToFScale() {
        ...
        TrackableCollection<Note> notes = model.getNotes();
        assertEquals("F-to-F scale should have 12 notes", 12, notes.size());
        ...
        // Verify last note is E5
        Note lastNote = notes.get(11);
        assertEquals("Last note should be E", "E", lastNote.getFriendlyName());
        assertEquals("Last note should be Natural", Accidental.Natural, lastNote.getAccidental());
    ...
    public void fToFScale_containsCorrectNotes() {
        ...
        // F-to-F scale: F, F#, G, G#, A, A#, B, C, C#, D, D#, E
        String[] expectedNames = {"F", "F", "G", "G", "A", "A", "B", "C", "C", "D", "D", "E"};
        Accidental[] expectedAccidentals = {
                Accidental.Natural, Accidental.Sharp,
                Accidental.Natural, Accidental.Sharp,
                Accidental.Natural, Accidental.Sharp,
    ...
    @Test
    public void cToC_allNotesAreOctave4() {
        PitchPipeModel model = new PitchPipeModel();

        TrackableCollection<Note> notes = model.getNotes();

        for (int i = 0; i < notes.size(); i++) {
            assertEquals("All C-to-C notes should be octave 4", 4, notes.get(i).getOctave());
        }
    }
