    ...
    @Test fun summary_note_bound_rendering_and_lifecycle() {
        withSilentPlayer {
            ActivityScenario.launch<TagDetailActivity>(fixture.detailIntent()).use { scenario ->
                ...
                val note = key.note
                settle()
                val idleFrame = Rect(key.left, key.top, key.right, key.bottom)
                capture("key-before")
                ...
                assertEquals("Held pitch keeps its visible face", idleFrame, Rect(key.left, key.top, key.right, key.bottom))
                capture("key-held")
                touch(key, MotionEvent.ACTION_UP)
                settle()
                assertFalse(note.isPlaying)
                checkRendered(key, false)
                assertEquals("Released pitch keeps its visible face", idleFrame, Rect(key.left, key.top, key.right, key.bottom))
