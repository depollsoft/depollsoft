...
class TagTracksFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        group.setOnCheckedChangeListener { g, checkedId ->
            ...
            val track = rb.tag as? RemoteLocation ?: return@setOnCheckedChangeListener
            if (selectedTrack != track) {
                selectedTrack = track
                // Confirm an actual part change, not a repeated selection.
                rb.confirmHaptic()
            }
        }
    ...
    override fun onPause() {
        // ViewPager keeps offscreen pages STARTED, so onStop may not run.
        stopMedia()
        super.onPause()
    }
