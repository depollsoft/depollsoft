...
public class PitchAudioTrackGenerator
{
      ...
      final Action<Integer> listener = new Action<Integer>()
      {
         ...
         public void invoke(Integer value)
         {
            try
            {
               ...
               {
                  short result = PitchAudioTrackGenerator.getAmplitude(
                        frequency, this.curTime, 1);
                  samples[x] = result;
                  if (numChannels == 2)
                     samples[x + 1] = result;
         ...
         int sampleRate, int bufferSizeInBytes)
   {
      synchronized (PitchAudioTrackGenerator.trackPool)
      {
         ...
         {
            StreamingAudioTrack track = PitchAudioTrackGenerator.trackPool
                  .poll();
         ...
         {
            return new StreamingAudioTrack(AudioManager.STREAM_MUSIC,
                  sampleRate, channelConfig, AudioFormat.ENCODING_PCM_16BIT,
