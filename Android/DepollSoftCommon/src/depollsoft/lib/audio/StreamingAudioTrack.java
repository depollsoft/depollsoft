...
public class StreamingAudioTrack extends AudioTrack
{
   private class TrackWatcherThread extends Thread
   {
      ...
      public void run()
      {
         while (this.keepGoing)
         {
            ...
            } while (StreamingAudioTrack.this.writtenFrames
                  - playbackHeadPosition < StreamingAudioTrack.this.bufferFrameThreshold);
            try
            {
               Thread.sleep(1);
            }
         ...
         int bufferSampleThreshold) throws IllegalArgumentException
   {
      ...
      this.numChannels = channelConfig == AudioFormat.CHANNEL_CONFIGURATION_STEREO ? 2
            : 1;
      this.bytesPerSample = audioFormat == AudioFormat.ENCODING_PCM_16BIT ? 2
            : 1;
   ...
   {
      this.writtenFrames += sizeInBytes / this.numChannels
            / this.bytesPerSample;
