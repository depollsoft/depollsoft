...
public class PitchPipeAppWidget extends AppWidgetProvider {
  ...
  public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
    ...
    for (int i = 0; i < ids.length; i++) {
      rv.setImageViewBitmap(ids[i], buildUpdateBitmap(context, model.getNotes().get(i)));
    ...
    PendingIntent launchPitchPipe = PendingIntent.getActivity(context, 0, new Intent(context, PitchPerfectActivity.class), PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
  ...
  private PendingIntent getIntentForNote(Note n, int index) {
    ...
    return PendingIntent.getService(PitchPerfectApplication.getAppContext(), index, i, PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_CANCEL_CURRENT);
  }
  ...
  public Uri buildUpdate(Context c, Note n) {
    ...
    Uri result = FileProvider.getUriForFile(c, c.getPackageName() + ".fileprovider", dest);
    for (PackageInfo pkg : c.getPackageManager().getInstalledPackages(0)) {
      c.grantUriPermission(pkg.packageName, result, Intent.FLAG_GRANT_READ_URI_PERMISSION);
    }
